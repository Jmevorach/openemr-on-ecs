...
def _parse_version(content: bytes) -> tuple[str | None, int | None]:
    fields = {
        key.decode("ascii"): (quoted or numeric).decode("utf-8", errors="strict").strip()
        for key, quoted, numeric in _VERSION_FIELD.findall(content)
    }
    ...
    database_version = fields.get("v_database")
    if database_version is None or not database_version.isdigit():
        return version, None
    return version, int(database_version)


def _is_active_content(path: PurePosixPath, mode: int, prefix: bytes) -> bool:
    ...
    return (
        bool(mode & 0o111)
        or bool(suffixes & _ACTIVE_CONTENT_SUFFIXES)
        or normalized_prefix.startswith(_ACTIVE_CONTENT_PREFIXES)
        or b"<?" in prefix
        or re.search(rb"(?:^|\n)[ \t]*#!", prefix) is not None
        or re.search(
...
def _valid_drive_key(content: bytes) -> bool:
    ...
    *,
    ...
    if parts[0] != "sites":
        ...
        if path.as_posix() == "version.php" and size <= 64 * 1024:
            facts["openemr_version"], facts["database_version"] = _parse_version(read_small_file())
    ...
    if relative and relative[0] == "documents":
        ...
        if not is_directory:
            ...
            if (
                ...
                and size <= 4096
                and _valid_drive_key(read_small_file()[:4097])
            ):
    ...
    if not is_directory and import_candidate:
        executable = _is_active_content(
            path,
            mode,
            read_small_file()[:4096],
        )
        if executable:
