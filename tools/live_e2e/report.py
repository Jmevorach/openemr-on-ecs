...
def render_markdown(history: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Import time is reported only when an approved run includes an import; it is otherwise shown as not",
            "  applicable.",
            "",
...
def _validate_run(value: Any) -> dict[str, Any]:
    ...
    import_duration = value.get("import_duration_seconds")
    if import_duration is not None and (
        isinstance(import_duration, bool) or not isinstance(import_duration, (int, float)) or import_duration < 0
    ):
        raise ToolError("Timing import duration must be null or non-negative")
    normalized["import_duration_seconds"] = None if import_duration is None else round(float(import_duration), 3)
...
def _latest_successful(run: dict[str, Any]) -> list[str]:
    import_duration = run.get("import_duration_seconds")
    import_display = "not applicable" if import_duration is None else _duration(float(import_duration))
    return [
