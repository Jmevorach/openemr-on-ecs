...
Floci maps RDS/Aurora ``EngineVersion`` onto a Docker tag. For Aurora MySQL
``8.0.mysql_aurora.3.12.0`` that becomes ``mysql:8.0.mysql_aurora.3.12.0``, which
is not published on Docker Hub. Pull the MySQL 8.0 community image and retag it
so Floci's Docker client can start the cluster without a registry 404.
"""
...
# Must stay aligned with openemr_ecs.constants.StackConstants.AURORA_MYSQL_ENGINE_VERSION.
DEFAULT_AURORA_ENGINE_VERSION = "8.0.mysql_aurora.3.12.0"
...
def aurora_mysql_floci_image(engine_version: str = DEFAULT_AURORA_ENGINE_VERSION) -> str:
    """Return the Docker image reference Floci will request for an Aurora engine version."""

    return f"mysql:{engine_version.strip()}"


def ensure_floci_runtime_images(
    *,
    engine_version: str = DEFAULT_AURORA_ENGINE_VERSION,
    ...
    prepared: list[str] = []
    target = aurora_mysql_floci_image(engine_version)
