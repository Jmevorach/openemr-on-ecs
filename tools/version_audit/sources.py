...
class HttpClient:
    ...
    def __init__(self, timeout_seconds: float = 12.0, max_bytes: int = 5_000_000):
...
class VersionSources:
    ...
    def _openemr_container(self, declaration: Declaration) -> Resolution:
        ...
        prerelease: list[str] = []
        declared_digest = str(declaration.metadata.get("arm64_digest", "")).strip()
        ...
        while url and pages < 20:
            ...
            for entry in data.get("results", []):
                ...
                arm64_digests = {
                    ...
                    and re.fullmatch(r"sha256:[0-9a-f]{64}", str(image.get("digest", "")))
                }
                if tag == declaration.current and len(arm64_digests) == 1:
                    observed_current_digest = next(iter(arm64_digests))
                try:
                    parsed = Version(tag)
                except InvalidVersion:
                    continue
        ...
        if declared_digest:
            digest_verified = (
                bool(re.fullmatch(r"sha256:[0-9a-f]{64}", declared_digest))
                and observed_current_digest == declared_digest
            )
            digest_note = (
                "; current ARM64 digest matches the immutable repository pin"
                if digest_verified
                else "; current ARM64 digest does not match the immutable repository pin"
            )
        return Resolution(
            latest=latest,
            latest_prerelease=_highest(prerelease, prerelease=True),
            source_url=releases_url,
            current_reference_verified=digest_verified,
            note=(
                "Requires both an ARM64 Docker tag and a matching non-draft, non-prerelease "
                "OpenEMR GitHub release; numeric development tags are excluded" + digest_note
            ),
        )
    ...
    def _python_toolchain(self) -> Resolution:
        ...
        data = self.client.get_json(url)
        stable_cycles: list[str] = []
        for release in data:
            cycle = str(release.get("cycle", ""))
            latest = str(release.get("latest", ""))
            if re.fullmatch(r"\d+\.\d+", cycle) and latest:
                stable_cycles.append(latest)
        latest_python = _highest(stable_cycles)
