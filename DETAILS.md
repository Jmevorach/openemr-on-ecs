 ...
 * `openemr_import_target`        Explicitly marks a newly initialized, non-production stack as eligible for the guarded import executor. Defaults to `false`; never enable it on a used or production stack. Follow [IMPORTING-OPENEMR.md](IMPORTING-OPENEMR.md), then return it to `false` after successful import cleanup.
