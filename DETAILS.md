...
Add Python dependencies to the appropriate requirements file and Node/CDK
tools through `package.json`; then regenerate and commit the corresponding
 ...
 * `openemr_import_target`          Creates the guarded, import-only staging bucket, EFS access point, dormant worker task, scoped permissions, and outputs only when explicitly set to `true`. Defaults to `false`; with the key absent or false, none of that import surface is synthesized. Use only when creating a fresh, empty, non-production target as described in [IMPORTING-OPENEMR.md](IMPORTING-OPENEMR.md).
 ...
 * `node_modules/.bin/cdk ls`          list all stacks in the app
 ...
 * `node_modules/.bin/cdk docs`        open CDK documentation
