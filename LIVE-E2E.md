...
The live E2E utility deploys a real, isolated OpenEMR stack, validates it,
records timings, and deletes it. Its AWS-facing `preflight`, `plan`, `run`, and
`cleanup` modes are intentionally local-only and are never invoked by GitHub
Actions; ordinary CI does exercise the utility's unit tests and local report
generation.
...
- `CI` must be unset and AWS-facing commands require an interactive local
  ...
  confirmations and ownership guards remain mandatory.
...
- Preflight expires after four hours and is bound to a clean Git commit, exact
  ...
  hosted-zone ID, tool versions, and the CDK/CDK-assets executable contents.
  `run` rechecks the supported Python/Node toolchain and both executable
  fingerprints before deployment.
...
For local scripting, `preflight`, `run`, and `cleanup` accept `--noninteractive`; all account, phrase, environment, cost, ownership, and CI guards remain active. `--json` emits machine-readable output on stdout, while phase progress and elapsed-time heartbeats always go to stderr. `--verbose` adds additional safe local diagnostics. Setting `CI` still blocks every AWS-facing mode.
...
run `npm ci` to install the pinned local AWS CDK and `cdk-assets` CLIs, and
ensure the worktree is clean. Node.js 22 and global CDK installations are not
supported by this runner. Replace every placeholder below; do not copy
