...
to ensure all features work correctly and pass cdk-nag and cfn-lint validation.
...
def test_configuration(
    ...
    Args:
        ...
        cfn_lint_command: Pinned cfn-lint executable
    ...
    if result.returncode == 0:
        ...
        templates = sorted((project_root / "cdk.out").glob("*.template.json"))
        if not templates:
            error_msg = "Synthesis produced no CloudFormation template"
            error(error_msg)
            return False, error_msg
        lint_result = subprocess.run(
            [str(cfn_lint_command), *(str(template) for template in templates)],
            cwd=project_root,
            capture_output=True,
            text=True,
        )
        if lint_result.returncode != 0:
            error_msg = f"cfn-lint failed for {config_name}"
            error(error_msg)
            if verbose:
                print("\n--- cfn-lint Output ---")
                print(lint_result.stdout)
                print(lint_result.stderr)
                print("--- End cfn-lint Output ---\n")
            return False, error_msg

        success(f"Synthesis and cfn-lint successful for {config_name}")
        return True, ""
...
def main() -> int:
    ...
    cfn_lint_command = Path(sys.executable).parent / "cfn-lint"
    ...
    if not cdk_command.is_file():
        ...
        return 1
    if not cfn_lint_command.is_file():
        error("Pinned cfn-lint is missing; install requirements-dev.txt")
        return 1
