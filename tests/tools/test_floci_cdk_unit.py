...
from tools.floci_cdk.deploy import emulator_environ, resolve_cdk_command
...
def test_floci_cdk_requires_repository_pinned_cli(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    (root / "cdk.json").write_text("{}\n", encoding="utf-8")
    (root / "openemr_ecs").mkdir()

    with pytest.raises(ToolError, match="Pinned CDK CLI"):
        resolve_cdk_command(root)
