...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("http://service.localhost:4566", True),
        ("http://10.0.0.5:4566", False),
        ("http://172.16.0.5:4566", False),
        ("http://172.15.0.5:4566", False),
        ("http://172.32.0.5:4566", False),
        ("http://floci.local:4566", False),
...
def test_local_endpoint_without_explicit_flag_is_not_floci(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(FLOCI_E2E_ENVIRONMENT, raising=False)
    assert is_floci_e2e_enabled("http://127.0.0.1:4566") is False


def test_ambient_endpoint_override_is_rejected_outside_floci(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(FLOCI_E2E_ENVIRONMENT, raising=False)
    ...
    with pytest.raises(ToolError, match="explicit guarded Floci mode"):
        LiveE2ERunner._cdk_emulator_environ()
...
def test_adapter_rejects_endpoint_without_explicit_emulated_mode() -> None:
    from tools.live_e2e.aws import LiveE2EAws
